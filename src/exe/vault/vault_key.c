...
static char *get_symmetric_key(user_impl_t *user) {
   ...
   if (n != SQLITE_OK) {
      log_error(LOG, "vault_key", "Error preparing statement: %s -- %s", sql, sqlite3_errstr(n));
   } else {
      n = sqlite3_bind_int64(stmt, 1, user->userid);
      if (n != SQLITE_OK) {
         log_error(LOG, "vault_key", "Error binding statement: %s -- %s", sql, sqlite3_errstr(n));
      } else {
         n = sqlite3_step(stmt);
         if (n == SQLITE_DONE) {
            ...
            char *saltedkey = decrypted(user->memory, hashkey, hashpwd);
            if (saltedkey != NULL) {
               char *b64key = unsalted(user->memory, keysalt, saltedkey);
               if (b64key != NULL) {
   ...
   if (result == NULL) {
      log_error(LOG, "vault_key", "Could not get encryption key for user %ld", (long int)user->userid);
   }
      ...
      if (n != SQLITE_OK) {
         ...
         if (n != SQLITE_OK) {
            ...
            do {
               ...
               case SQLITE_OK:
                  if (result != NULL) {
                     log_error(LOG, "vault_key", "Error: multiple entries for key %ld", (long int)this->keyid);
                     ...
                     char *decvalue = decrypted(this->memory, value, enckey);
                     if (decvalue != NULL) {
                        result = unsalted(this->memory, salt, decvalue);
               ...
               case SQLITE_DONE:
                  ...
                  break;
               default:
                  log_error(LOG, "vault_key", "Error stepping statement: %s -- %s", sql, sqlite3_errstr(n));
                  done = 1;
               }
            } while (!done);
         }
...
static int vault_key_set_password(key_impl_t *this, const char *password) {
   ...
   if (enckey != NULL) {
      ...
      if (keysalt == NULL) {
         log_error(LOG, "vault_key", "Could not allocate salt");
      } else {
         char *saltedpwd = salted(this->memory, keysalt, password);
         if (saltedpwd == NULL) {
            log_error(LOG, "vault_key", "Could not allocated salted password");
         }
         encpwd = encrypted(this->memory, saltedpwd, enckey);
      }
   }
   if (encpwd == NULL) {
      log_error(LOG, "vault_key", "Could not encrypt password");
   } else {
      assert(keysalt != NULL);
      ...
      if (n != SQLITE_OK) {
         ...
         n = sqlite3_finalize(stmt);
         if (n != SQLITE_OK) {
            log_warning(LOG, "vault_key", "Error in finalize: %s", sqlite3_errstr(n));
         }
      }
   }
   return result;
